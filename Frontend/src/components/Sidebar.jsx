...
import { Ellipsis, Pencil, Trash, Trash2, X } from "lucide-react";
...
const Sidebar = () => {
  ...
  const showSidebar = useSelector((state) => state.showSidebar.items);
  ...
  return (
    <div
      className={`absolute top-0  z-99 left-0 h-screen flex flex-col justify-between bg-[#1A1A1A] border border-[#2A2A2A] text-orange-500 font-bold text-2xl p-3 transition-all duration-300
        ${
          showSidebar
            ? " md:w-[5rem] w-[4rem] transform translate-x-[-100%]"
            : "md:w-[20rem] transform translate-x-0 w-[15rem]"
        }`}
    >
      ...
      <div>
        {!showSidebar && (
          <div className="flex items-center justify-between w-full mb-10 px-5">
            <img
            ...
            />
            <button
              onClick={setShowSidebarHandler}
              className="text-white px-2 hover:text-orange-500 cursor-pointer"
            >
              <X />
            </button>
          </div>
        ...
        <button
          ...
          className={`mt-5 font-semibold text-white py-2 px-4  rounded-lg text-lg flex items-center justify-center transition-all duration-300 ease-in-out transform
            ...
            ${
              showSidebar
                ? "md:w-[3rem] md:h-[3rem] h-8 w-8 rounded-full"
                : "w-[90%]"
            }`}
        >
          {showSidebar ? (
          ...
          {chats.map((chat) => (
            <li
              ...
              className={`py-2 px-4 inline-flex items-center justify-between w-full rounded-lg cursor-pointer text-zinc-300 transition-colors truncate
                ${
                  activeChat === chat._id && !showSidebar
                    ? "border-1 bg-zinc-800 border-zinc-600"
                    : ""
                }`}
            >
              {!showSidebar && chat.chatName}
              {!showSidebar && (
                <Ellipsis
